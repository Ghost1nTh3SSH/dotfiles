...
set_term_config() {
	cp -f ~/.config/kitty/themes/cthulhu.conf ~/.config/kitty/current_theme.conf
}
...
# Set rofi launcher
set_rofi_config() {
    cp -f ~/.config/bspwm/themes/cthulhu/launcher.rasi ~/.config/bspwm/scripts/RiceSelector.rasi
}
...
set_p10k_config() {
	sed -i "$HOME"/.p10k.zsh \
		...
		-e "s/POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=.*/POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=070/g" \
		-e "s/POWERLEVEL9K_DIR_ANCHOR_FOREGROUND=.*/POWERLEVEL9K_DIR_ANCHOR_FOREGROUND=070/g"
