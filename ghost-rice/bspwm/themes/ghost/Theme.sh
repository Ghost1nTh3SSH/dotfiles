...
set_term_config() {
	cp -f ~/.config/kitty/themes/ghost.conf ~/.config/kitty/current_theme.conf
}
...
# Set rofi launcher
set_rofi_config() {
    cp -f ~/.config/bspwm/themes/ghost/launcher.rasi ~/.config/bspwm/scripts/RiceSelector.rasi
}
...
set_p10k_config() {
	sed -i "$HOME"/.p10k.zsh \
		...
		-e "s/POWERLEVEL9K_OS_ICON_CONTENT_EXPANSION=.*/POWERLEVEL9K_OS_ICON_CONTENT_EXPANSION='󰔷 󰫴󰫵󰬹󰬀󰬁'/g" \
 		-e "s/POWERLEVEL9K_OS_ICON_FOREGROUND=.*/POWERLEVEL9K_OS_ICON_FOREGROUND=038/g" \
		-e "s/POWERLEVEL9K_DIR_FOREGROUND=.*/POWERLEVEL9K_DIR_FOREGROUND=038/g" \
		-e "s/POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=.*/POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=038/g" \
